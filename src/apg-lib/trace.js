...
module.exports = function () {
    ...
    let circular = new (require("./circular-buffer.js"))();
    ...
    let PHRASE_END = '<span class="' + style.CLASS_LINEEND + '">&bull;</span>';
    let PHRASE_CONTINUE = '<span class="' + style.CLASS_LINEEND + '">&hellip;</span>';
    let PHRASE_EMPTY = '<span class="' + style.CLASS_EMPTY + '">&#120634;</span>';
    /* filter the non-RNM & non-UDT operators */
    let initOperatorFilter = function () {
        let setOperators = function (set) {
        ...
        let items = 0;
        for (let name in that.filter.operators) {
            items += 1;
        }
        if (items === 0) {
    ...
    let initRuleFilter = function () {
        let setRules = function (set) {
            operatorFilter[id.RNM] = set;
            operatorFilter[id.UDT] = set;
            let count = rules.length + udts.length;
            ruleFilter.length = 0;
        ...
        ruleFilter.length = 0;
        items = 0;
        for (let name in that.filter.rules) {
            items += 1;
        }
        if (items === 0) {
    ...
    let filterRecords = function (record) {
        if (lastRecord === -1) {
            return true;
        }
    ...
    this.up = function (op, state, offset, length, anchor, lookAround) {
        if (filterRecords(filteredRecords) && filterOps(op)) {
            let thisLine = filteredRecords;
            let thatLine = recordStack.pop();
            let thatRecord = circular.getListIndex(thatLine);
            if (thatRecord !== -1) {
    ...
    let toTreeObj = function () {
        ...
        function display(node, offset, comma) {
            ...
            obj.children = [];
            for (let i = 0; i < node.children.length; i += 1) {
                let c = i === node.children.length - 1 ? false : true;
        ...
        let nodeId = -1;
        let branch = [];
        let root, node, dummy, parent, record;
        ...
        circular.forEach(function (lineIndex, index) {
            ...
            if (firstRecord) {
                ...
                if (record.depth > 0) {
                    ...
                    let num = record.dirUp ? record.depth + 1 : record.depth;
    ...
    let htmlHeader = function (mode, caption) {
        /* open the page */
    ...
    let htmlFooter = function () {
        ...
        footer +=
            '&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="' +
            style.CLASS_ACTIVE +
            '">&darr;</span>&nbsp;&nbsp;phrase opened<br>\n';
        footer +=
            '&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="' +
            style.CLASS_MATCH +
            '">&uarr;M</span> phrase matched<br>\n';
        footer +=
            '&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="' +
            style.CLASS_EMPTY +
            '">&uarr;E</span> empty phrase matched<br>\n';
        footer +=
            '&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="' +
            style.CLASS_NOMATCH +
            '">&uarr;N</span> phrase not matched<br>\n';
        footer +=
            "operator&nbsp;-&nbsp;ALT, CAT, REP, RNM, TRG, TLS, TBS<sup>&dagger;</sup>, UDT, AND, NOT, BKA, BKN, BKR, ABG, AEN<sup>&Dagger;</sup><br>\n";
        footer +=
            "phrase&nbsp;&nbsp;&nbsp;-&nbsp;up to " + MAX_PHRASE + " characters of the phrase being matched<br>\n";
        footer +=
            '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="' +
            style.CLASS_MATCH +
    ...
    let htmlTable = function (mode) {
        if (rules === null) {
            return "";
        }
        let html = "";
        let thisLine, thatLine, lookAhead, lookBehind, lookAround, anchor;
        html += "<tr><th>(a)</th><th>(b)</th><th>(c)</th><th>(d)</th><th>(e)</th><th>(f)</th>";
        html += "<th>operator</th><th>phrase</th></tr>\n";
        circular.forEach(function (lineIndex, index) {
            let line = records[lineIndex];
            ...
            if (line.opcode.type === id.BKR) {
                let casetype = line.opcode.bkrCase === id.BKR_MODE_CI ? "%i" : "%s";
                let modetype = line.opcode.bkrMode === id.BKR_MODE_UM ? "%u" : "%p";
    ...
    let displayTrg = function (mode, op) {
        let html = "";
        if (op.type === id.TRG) {
    ...
    let displayRep = function (mode, op) {
        let html = "";
        if (op.type === id.REP) {
            if (mode === MODE_HEX) {
                let hex = op.min.toString(16).toUpperCase();
    ...
    let displayTbs = function (mode, op) {
        let html = "";
        if (op.type === id.TBS) {
            let len = Math.min(op.string.length, MAX_TLS * 2);
            if (mode === MODE_HEX || mode === MODE_UNICODE) {
                html += mode === MODE_HEX ? "%x" : "U+";
    ...
    let displayTls = function (mode, op) {
        let html = "";
        if (op.type === id.TLS) {
            let len = Math.min(op.string.length, MAX_TLS);
            if (mode === MODE_HEX || mode === MODE_DEC) {
                let charu, charl, base;
                if (mode === MODE_HEX) {
                    html = "%x";
                    base = 16;
                } else {
    ...
    let displayBehind = function (mode, chars, state, index, length, anchor) {
        let html = "";
        let beg1, len1, beg2, len2;
        let lastchar = PHRASE_END;
        let spanBehind = '<span class="' + style.CLASS_LOOKBEHIND + '">';
        let spanRemainder = '<span class="' + style.CLASS_REMAINDER + '">';
        let spanend = "</span>";
        let prev = false;
        switch (state) {
            case id.EMPTY:
    ...
    let displayAhead = function (mode, chars, state, index, length) {
        let spanAhead = '<span class="' + style.CLASS_LOOKAHEAD + '">';
        return displayForward(mode, chars, state, index, length, spanAhead);
    };
    /* display phrases matched in normal parsing mode */
    let displayNone = function (mode, chars, state, index, length) {
        let spanAhead = '<span class="' + style.CLASS_MATCH + '">';
        return displayForward(mode, chars, state, index, length, spanAhead);
    };
    let displayForward = function (mode, chars, state, index, length, spanAhead) {
        let html = "";
        let beg1, len1, beg2, len2;
        let lastchar = PHRASE_END;
        let spanRemainder = '<span class="' + style.CLASS_REMAINDER + '">';
    ...
    let subPhrase = function (mode, chars, index, length, prev) {
        if (length === 0) {
            return "";
        }
        let phrase = "";

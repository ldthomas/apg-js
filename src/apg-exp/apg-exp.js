...
module.exports = function (input, flags, nodeHits, treeDepth) {
    ("use strict;");
    let _this = this;
    let thisFileName = "apg-exp: ";
    let errorName = thisFileName;
    let apglib = require("../apg-lib/node-exports.js");
    let execFuncs = require("./exec.js");
    let replaceFuncs = require("./replace.js");
    let resultFuncs = require("./result.js");
    let splitFuncs = require("./split.js");
    let setFlags = require("./flags.js");
    let sabnfGenerator = require("./sabnf-generator.js");
    let readonly = {
        writable: false,
        enumerable: false,
        configurable: true,
    ...
    let ApgExpError = function (msg, t, h) {
        this.message = msg;
    ...
    let checkParserUdts = function (errorName) {
        let udterrors = [];
        let error = null;
        for (let i = 0; i < priv.grammarObject.udts.length; i += 1) {
            let lower = priv.grammarObject.udts[i].lower;
    ...
    this.split = function (str, limit) {
        ...
        if (this.unicode) {
            throw new ApgExpError(
                errorName + "cannot do string split in 'unicode' mode. Insure that 'u' flag is absent."
            );
        }
    ...
    this.include = function (list) {
        ...
        if (list === undefined || list == null || (typeof list === "string" && list.toLowerCase() === "all")) {
            /* set all to true */
            for (let name in priv.grammarObject.callbacks) {

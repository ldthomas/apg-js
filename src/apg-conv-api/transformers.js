...
let NON_SHORTEST = 0xfffffffc;
let TRAILING = 0xfffffffd;
let RANGE = 0xfffffffe;
let ILL_FORMED = 0xffffffff;

/* mask[n] = 2**n - 1, ie. mask[n] = n bits on. e.g. mask[6] = %b111111 */
let mask = [0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023];
...
let base64chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".split("");
...
exports.utf8 = {
    ...
    decode: function (buf, bom) {
        ...
        function bytes3(b1, b2, b3) {
            ...
            let x = ((b1 & mask[4]) << 12) + ((b2 & mask[6]) << 6) + (b3 & mask[6]);
            if (x < 0x800) {
                return NON_SHORTEST;
            }
...
exports.utf32be = {
    encode: function (chars) {
        let buf = Buffer.alloc(chars.length * 4);
        let i = 0;
        chars.forEach(function (char) {
            if ((char >= 0xd800 && char <= 0xdfff) || char > 0x10ffff) {
                throw new RangeError(
                    "utf32be.encode: UTF32BE character code out of range: char[" + i / 4 + "]: " + char
                );
            }
    ...
    decode: function (buf, bom) {
        ...
        if (buf.length % 4 > 0) {
            throw new RangeError(
                "utf32be.decode: UTF32BE byte length must be even multiple of 4: length: " + buf.length
            );
        }
        let chars = [];
        let i = bom ? 4 : 0;
        for (; i < buf.length; i += 4) {
            let char = (buf[i] << 24) + (buf[i + 1] << 16) + (buf[i + 2] << 8) + buf[i + 3];
            if ((char >= 0xd800 && char <= 0xdfff) || char > 0x10ffff) {
                throw new RangeError(
                    "utf32be.decode: UTF32BE character code out of range: char[" + i / 4 + "]: " + char
                );
            }
            chars.push(char);
        }
...
exports.utf32le = {
    encode: function (chars) {
        ...
        chars.forEach(function (char) {
            if ((char >= 0xd800 && char <= 0xdfff) || char > 0x10ffff) {
                throw new RangeError(
                    "utf32le.encode: UTF32LE character code out of range: char[" + i / 4 + "]: " + char
                );
            }
            buf[i++] = char & mask[8];
            buf[i++] = (char >> 8) & mask[8];
    ...
    decode: function (buf, bom) {
        ...
        if (buf.length % 4 > 0) {
            throw new RangeError(
                "utf32be.decode: UTF32LE byte length must be even multiple of 4: length: " + buf.length
            );
        }
        let chars = [];
        let i = bom ? 4 : 0;
        for (; i < buf.length; i += 4) {
            let char = (buf[i + 3] << 24) + (buf[i + 2] << 16) + (buf[i + 1] << 8) + buf[i];
            if ((char >= 0xd800 && char <= 0xdfff) || char > 0x10ffff) {
                throw new RangeError(
                    "utf32le.encode: UTF32LE character code out of range: char[" + i / 4 + "]: " + char
                );
...
exports.uint16be = {
    encode: function (chars) {
        let buf = Buffer.alloc(chars.length * 2);
        let i = 0;
        chars.forEach(function (char) {
            if (char > 0xffff) {
                throw new RangeError(
                    "uint16be.encode: UINT16BE character code out of range: char[" + i / 2 + "]: " + char
                );
            }
            buf[i++] = (char >> 8) & mask[8];
            buf[i++] = char & mask[8];
    ...
    decode: function (buf) {
        if (buf.length % 2 > 0) {
            throw new RangeError(
                "uint16be.decode: UINT16BE byte length must be even multiple of 2: length: " + buf.length
            );
        }
        let chars = [];
...
exports.uint16le = {
    encode: function (chars) {
        ...
        chars.forEach(function (char) {
            if (char > 0xffff) {
                throw new RangeError(
                    "uint16le.encode: UINT16LE character code out of range: char[" + i / 2 + "]: " + char
                );
            }
            buf[i++] = char & mask[8];
            buf[i++] = (char >> 8) & mask[8];
    ...
    decode: function (buf) {
        if (buf.length % 2 > 0) {
            throw new RangeError(
                "uint16le.decode: UINT16LE byte length must be even multiple of 2: length: " + buf.length
            );
        }
        let chars = [];
        for (let i = 0; i < buf.length; i += 2) {
...
exports.uint32be = {
    ...
    decode: function (buf) {
        if (buf.length % 4 > 0) {
            throw new RangeError(
                "uint32be.decode: UINT32BE byte length must be even multiple of 4: length: " + buf.length
            );
        }
        let chars = [];
...
exports.uint32le = {
    ...
    decode: function (buf) {
        ...
        if (buf.length % 4 > 0) {
            throw new RangeError(
                "uint32le.decode: UINT32LE byte length must be even multiple of 4: length: " + buf.length
            );
        }
        let chars = [];
        for (let i = 0; i < buf.length; i += 4) {
...
exports.escaped = {
    ...
    decode: function (buf) {
        ...
        function getx(i, len, buf) {
            let ret = { char: null, nexti: i + 2, error: true };
            if (i + 1 < len) {
                if (isHex(buf[i]) && isHex(buf[i + 1])) {
                    let str = String.fromCodePoint(buf[i], buf[i + 1]);
        ...
        function getu(i, len, buf) {
            let ret = { char: null, nexti: i + 4, error: true };
            if (i + 3 < len) {
                if (isHex(buf[i]) && isHex(buf[i + 1]) && isHex(buf[i + 2]) && isHex(buf[i + 3])) {
                    let str = String.fromCodePoint(buf[i], buf[i + 1], buf[i + 2], buf[i + 3]);
                    ret.char = parseInt(str, 16);
                    if (!isNaN(ret.char)) {
                        ret.error = false;
                    }
...
exports.base64 = {
    encode: function (buf) {
        ...
        let base64 = Buffer.alloc(units * 4);
        if (tail > 0) {
            units -= 1;
        }
        i = 0;
    ...
    decode: function (codes) {
        ...
        let val = validate(codes);
        let tail = val.tail;
        let base64 = val.buf;
        let i, j, n;
        let units = base64.length / 4;
        let buf = Buffer.alloc(units * 3 - tail);
        if (tail > 0) {
            units -= 1;
        }
        j = 0;
    ...
    toString: function (buf) {
        ...
        for (let i = 0; i < buf.length; i += 4) {
            for (let j = i; j < i + 4; j += 1) {
                if (!validate(buf[j])) {
                    throw new RangeError(
                        "base64.toString: buf[" + j + "]: " + buf[j] + " : not valid base64 character code"
                    );
                }
            }
            buildLine(

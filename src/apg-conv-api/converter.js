...
"use strict;";
let _this = this;
let trans = require("./transformers.js");
...
let UINT8 = "UINT8";
let UINT16 = "UINT16";
let UINT16LE = "UINT16LE";
let UINT16BE = "UINT16BE";
let UINT32 = "UINT32";
let UINT32LE = "UINT32LE";
let UINT32BE = "UINT32BE";
let ESCAPED = "ESCAPED";
let STRING = "STRING";

/* private functions */
// Find the UTF8 BOM, if any.
let bom8 = function (src) {
    src.type = UTF8;
    let buf = src.data;
    src.bom = 0;
    if (buf.length >= 3) {
...
let validateSrc = function (type, data) {
    function getType(type) {
        let ret = {
            type: "",
            base64: false,
        };
        let rx = /^(base64:)?([a-zA-Z0-9]+)$/i;
        let result = rx.exec(type);
        if (result) {
            if (result[2]) {
    ...
    let ret = getType(type.toUpperCase());
    if (ret.base64) {
        /* handle base 64 */
        if (ret.type === STRING) {
...
let validateDst = function (type, chars) {
    function getType(type) {
        let fix, rem;
        let ret = {
            crlf: false,
...
let encode = function (type, chars) {
    switch (type) {
        case UTF8:
